...
;   InstallTimer1Handler - add the debouncing timer 1 event handler to the IVT.
;   InitTimer1           - initialize timer1 interrupts.
;
;
; Revision History:
;
...
;    11/5/15   David Qu          moved code to timer.asm.
...
; external function declarations
        ...
        EXTRN   MultiplexDisplay:NEAR ;Display event handler.
        EXTRN   HandleKeypad:NEAR     ;Keypad event handler.
...
InstallTimer0Handler  PROC    NEAR
        ...
        MOV     ES, AX
                                ;store the vector
        MOV     ES: WORD PTR (Tmr0VecOffset), OFFSET(MultiplexDisplay)
        MOV     ES: WORD PTR (Tmr0VecSeg), SEG(MultiplexDisplay)
...
; InitTimer1
;
; Description:       Initialize the 80188 Timer 1.  Timer 1 is initialized
;                    to generate interrupts every millisecond.
;                    The interrupt controller is also initialized to allow the
;                    timer interrupts.  Timer #1 then counts COUNTS_PER_MS 
;                    clocks to generate the interrupts at 1 kHz, which is good
;                    enough to debounce the keypad.
;
; Operation:         The appropriate values are written to the timer control
;                    registers in the PCB.  Also, the timer count registers
;                    are reset to zero.  Finally, the interrupt controller is
...
InitTimer1      PROC    NEAR
				PUBLIC	InitTimer1
                                ...
                                ;initialize Timer #1 for 1 ms interrupts
        MOV     DX, Tmr1Count   ;initialize the count register to 0
        XOR     AX, AX
        OUT     DX, AL

        MOV     DX, Tmr1MaxCntA     ;setup max count of timer clocks
        MOV     AX, COUNTS_PER_MS   ;so can time digits
        OUT     DX, AL
...
InstallTimer1Handler  PROC    NEAR
        ...
        MOV     ES, AX
                                ;store the vector
        MOV     ES: WORD PTR (Tmr1VecOffset), OFFSET(HandleKeypad)
        MOV     ES: WORD PTR (Tmr1VecSeg), SEG(HandleKeypad)
